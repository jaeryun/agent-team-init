...
`.claude/team-config.json`이 존재하는지 확인하고, 모델 설정을 읽는다:
...
파일이 있으면 `modelConfig`를 추출한다:

```bash
jq '.modelConfig // {}' .claude/team-config.json
```

`modelConfig`가 없거나 키가 없는 역할은 스폰 시 `sonnet`을 기본값으로 사용한다.
...
`modelConfig.orchestrator` 값을 읽어 `model=` 파라미터로 전달한다. (없으면 `sonnet`)
...
Agent(
  ...
  model="{modelConfig.orchestrator | 기본값: sonnet}",
  prompt="..."
)
```
...
Agent(
  ...
  model="{modelConfig['engineering-backend-architect'] | 기본값: sonnet}"
)
```
...
Agent(
  ...
  model="{modelConfig['engineering-frontend-developer'] | 기본값: sonnet}"
)
```
...
Agent(
  ...
  model="{modelConfig['engineering-database-optimizer'] | 기본값: sonnet}"
)
```
...
Agent(
  ...
  model="{modelConfig['engineering-devops-automator'] | 기본값: sonnet}"
)
```
...
스폰된 멤버:
  🎛️  orchestrator — Agents Orchestrator  (파이프라인 실행)  [모델: {orchestrator 모델}]
  [선택된 도메인 멤버 목록 + 각 모델명]
